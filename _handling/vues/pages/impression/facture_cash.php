...
$s="select * from  facture_imprime where Mouv='$num_mouv'"; 
...
{	
	...
	echo ('<script type="application/javascript" language="javascript">'.
		'location.href="'.$url."'".
		'</script>');
	echo(utf8_decode("<center><div align='center' style='text-align:center; font-weight:bold; font-size:25px; background-color:red; width:65%; border-radius:20px; padding:20px; color:white'>Désolé, cette facture est déjà imprimée</div></center>"));
...
}else
{
	...
	$s="select * from facture_imprime order by Id_fact desc"; $e=mysqli_query($bdd,$s); $t=mysqli_fetch_array($e);
		$num=$t['N_ordre']+1;
		$num_facture=format_nbre($num)."/RDA.FZQA/".date('n.Y');
	//====================================== DONNEES DE LA TABLE ========================
	...
	$pdf->Text(10, 32, utf8_decode("----------------------------------------------------------------------"));
			...
			//=========================================INSERTION DU N° DE LA FACTURE
				...
				if($n==0)
				{
					mysqli_query($bdd,"insert into facture_imprime values('','$dt','$heure','$num','$num_facture','','$_mouv','V','FACT')");
				}
			//=============================================================
...
if($mouvement['tot_pax_d']>0)
{
	...
	$pdf->Text(10, 32, utf8_decode("----------------------------------------------------------------------"));
			...
			//=========================================INSERTION DU N° DE LA FACTURE
				...
				$fact_rdb="select * from rdb_facture_imprime order by Id_impr desc"; 
	...
	//============== insertion dans sql=========================
	mysqli_query($bdd,"insert into rdb_facture_imprime values('','$dt','$num_mouv','$id_us')");
}
//=========================================================================================================	
//============================ FACTURE REDEVANCE IDF =================================
if($mouvement['tot_red_fret']>0)
{
	...
	$pdf->Text(10, 32, utf8_decode("----------------------------------------------------------------------"));
			...
			//=========================================INSERTION DU N° DE LA FACTURE
				...
				$s="select * from facture_imprime order by Id_fact desc"; $e=mysqli_query($bdd,$s); $t=mysqli_fetch_array($e);
				$num=$t['N_ordre']+1;
				$num_facture=format_nbre($num)."/RDS.FZQA/".date('n.Y');				
