<?php
	...
	$pdf->Text(10, 32, utf8_decode("----------------------------------------------------------------------"));
			...
			$s="select * from rva_facturation2.handling_paiement where Date_paie between '$dt1' and '$dt2'";
			$e=$m->cnx->query($s);
			$t=($e->fetchAll());
			$n=count($t);
			//===============================================================================
			...
			$pdf->Cell(280,7, utf8_decode("RAPPORT PAIEMENT HANDLING"),'','','C'); 
			$pdf->Ln();
			$pdf->SetFont('Arial','',9);
			...
			}else
			{
				...
				$pdf->Cell(54,7, utf8_decode("FACTURE"),'TB','','C'); 
				$pdf->Cell(23,7, utf8_decode("PERCEPTEUR"),'TB','','C');
				...
				foreach($t as $row)
				{
							...
							$fact=$m->handling_facture($row['Fact_paie']);
							$paie=$m->paie_handling_detail($row['Fact_paie']);
							$pdf->Ln();
							
							$pdf->Cell(14,7, utf8_decode($paie['dt_paie']),'','','C'); 
							$pdf->Cell(54,7, utf8_decode($paie['num_fact']),'','','C'); 
							$pdf->Cell(23,7, utf8_decode($paie["percepteur"]),'','','C'); 
							...
							$pdf->Cell(20,7, $m->arrondie($paie['mht']),'','','C'); 
							$pdf->Cell(15,7, $m->arrondie($paie['tva']),'','','C');
							$pdf->Cell(30,7, $m->arrondie($paie['mttc']),'','','C'); 
				...
				$pdf->SetFont('Arial','B',8);
				$pdf->Ln();
				$pdf->Cell(210,7, utf8_decode("TOTAL   "),'TB','','C'); 
