<?php
	...
	$pdf->Text(10, 32, utf8_decode("----------------------------------------------------------------------"));
			//=========================== CREATION N° FACTURE===============================
				if($client!=="T")
				{
					$s="select * from rva_facturation2.handling_facturation where Date_dep between '$dt1' and '$dt2' and Handleur='$client'";
				}else
				{
			...
			$pdf->Cell(280,7, utf8_decode("RAPPORT DES FACTURES NON PAYEES "),'','','C'); 
			$pdf->Ln();
			$pdf->SetFont('Arial','',9);
			...
			}else
			{
				...
				foreach($t as $row)
				{
					//============== VERIFICATION DANS LA TABLE DE PAIEMENT===================
						$id_fact=$row['Id_fact'];
						$s2="select * from rva_facturation2.handling_paiement where Fact_paie='$id_fact'";
						$e2=$m->cnx->query($s2);
						$t2=($e2->fetchAll());
						$n2=count($t2);
						if($n2!==0)
