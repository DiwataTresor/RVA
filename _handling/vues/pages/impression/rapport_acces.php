<?php
	...
	$dt1=$_GET['dt1'];
	$dt2=$_GET['dt2'];
	$s="select * from acces,type_acces where acces.Type_acc=type_acces.Id_acc and acces.Type_acc='$acces' and Date_perc between '$dt1' and '$dt2'";
	$e=mysqli_query($bdd,$s); $n=mysqli_num_rows($e); $t=mysqli_fetch_array($e);
	if($n==0)
	{		
	...
	}else
	{
		...
		$pdf->Cell(180,5,("RELEVE DE RECETTES ".$t["Designation_acc"]),'','','C');	
		$pdf->Ln();
		$pdf->SetFont('Arial','BU',9);
		$pdf->Cell(180,5,("DU ".Datemysqltofr($dt1)." AU ".Datemysqltofr($dt1)),'','','C');	
			$pdf->Ln(10);
			$pdf->SetFont('Arial','',8);
			...
			do
			{
				...
				$pdf->Cell(20,5,Datemysqltofr($t['Date_perc']),'LTRB','','C');
				$pdf->Cell(20,5,arrondie($mhtcdf),'TRB','','R');
				$pdf->Cell(20,5,arrondie($tvacdf),'TRB','','R');	
				$pdf->Cell(20,5,arrondie($mhtusd),'TRB','','R');
				$pdf->Cell(20,5,arrondie($tvausd),'TRB','','R');	
				$pdf->Cell(15,5,arrondie($arrondiecdf),'TRB','','R');	
				$pdf->Cell(15,5,arrondie($arrondieusd),'TRB','','R');	
				$pdf->Cell(20,5,arrondie($mttcdf),'TRB','','R');	
				$pdf->Cell(30,5,arrondie($mttusd),'TRB','','R');	
				...
				$pdf->Cell(20,5,arrondie($cdf),'TRB','','R');
				$pdf->Cell(20,5,arrondie($tvacdf_total),'TRB','','R');	
				$pdf->Cell(20,5,arrondie($usd),'TRB','','R');
				$pdf->Cell(20,5,arrondie($tvausd_total),'TRB','','R');	
				$pdf->Cell(15,5,arrondie($arrondiecdf_total),'TRB','','R');	
				$pdf->Cell(15,5,arrondie($arrondieusd_total),'TRB','','R');	
				$pdf->Cell(20,5,arrondie($ttccdf_total),'TRB','','R');	
				$pdf->Cell(30,5,arrondie($ttcusd_total),'TRB','','R');		
		$pdf->Output();
	}
?>