	...
	$.ajax({
		...
		success:function(data)
		{
			...
			if(data==1)
			{
				...
				if(tabl=="handling_paiement")
				{
					liste_paiement_handling();
				}
...
function supprimer_mouv()
{
	...
	$.ajax({
		...
		success:function(data)
		{
			...
			{
				$(".resultat")
				.html("<div class='alert alert-danger center fs-2'>Cette facture à déjà été imprimée</div>");
			}else
			{
				$(".resultat")
				.html("<div class='w3-border p-1'>"+
						"<div class='p-1'>CLIENT "+r["ta"][0]['Nom_cli']+"</div>"+
