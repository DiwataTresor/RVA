...
class main
{
	...
	function __construct()
	{
		if(date("Y-m-d")<"2021-12-31")
		{
			...
			//$this->cnx=new PDO("sqlsrv:Server=DESKTOP-KHFDC29\SERVER2;Database=rva_facturation2", "", "");
		}
	...
	{		
		...
		//==RED PASSGER 
			...
			$s_red_pax_arr="select * from rva_facturation2.escale,rva_facturation2.pt_emplacement 
							where
							...
							and 
								Id_mouv='$num_mouv'";
			$e_red_pax_arr=$this->cnx->query($s_red_pax_arr);
			$t_red_pax_arr=$e_red_pax_arr->fetchAll();
			$red_pass_a=0;
			$red_pass_d=0;
			foreach ($t_red_pax_arr as $row) {
				if(substr($row['Code_pt'],0,2)=="FZ")
				{
					//$red_pass_a=0;
					
				}else
				{
					$red_pass_a+=($row["Ad"]+$row["Ch"]+$row["Inf"])*$ttarif[0]['Redpass_res_int'];
				}
			}

			$s_red_pax_dep="select * from rva_facturation2.escale,rva_facturation2.pt_emplacement 
							where
								escale.Prov_dest=pt_emplacement.Id_pt
							and 
								Sens='D' 
							and 
								Id_mouv='$num_mouv'";
			$e_red_pax_dep=$this->cnx->query($s_red_pax_dep);
			$t_red_pax_dep=$e_red_pax_dep->fetchAll();
			$red_pass_a=0;
			$red_pass_d=0;
			foreach ($t_red_pax_dep as $row) {
				if(substr($row['Code_pt'],0,2)=="FZ")
				{
					$red_pass_d+=($row["Ad"]+$row["Ch"]+$row["Inf"])*$ttarif[0]['Redpass_res_nat'];
				}else
				{
					$red_pass_d+=($row["Ad"]+$row["Ch"]+$row["Inf"])*$ttarif[0]['Redpass_res_int'];
				}
			}
		...
		//== RED SURETE
			...
			foreach ($t_red_pax_arr as $row) {
				...
				{
					$red_sur_a+=($row["Ad"]+$row["Ch"]+$row["Inf"])*$ttarif[0]['Redsur_inter'];
				}
			}
			/*En cas d'erreur, à retirer*/
			if($td[0]["Date_mouv"]>"2021-09-08")
			{
				if($td[0]['Type_mouv']=='N'){
					$red_sur_d=0;
				}else
				{
					...
					if(substr($row['Code_pt'],0,2)=="FZ")
						{
							//$red_sur_d+=($row["Ad"]+$row["Ch"]+$row["Inf"])*$ttarif[0]['Redsur_nat'];
							$red_sur_d=0;
						}else
						{
							$red_sur_d+=($row["Ad"]+$row["Ch"]+$row["Inf"])*$ttarif[0]['Redsur_inter'];
						}
					}
				}
			}else
		...
		//== RED SECURITE
			...
			if($td[0]["Date_mouv"]>"2021-09-08")
			{
				if(trim($td[0]['Type_mouv'])=='N'){
					$red_sec_d=0;
				}else{
					foreach ($t_red_pax_dep as $row) {
						if(substr($row['Code_pt'],0,2)=="FZ")
						{
							if(($row["Ad"]+$row["Ch"]+$row["Inf"])<20)
							{
								if($ta[0]['Nbre_siege']<20)
								{
								...
								{
									$red_sec_d+=($ttarif[0]['Redsecurite']*($row["Ad"]+$row["Ch"]+$row["Inf"]));
								}
							}else
							{
								$red_sec_d+=($ttarif[0]['Redsecurite']*($row["Ad"]+$row["Ch"]+$row["Inf"]));
		...
		//======= DONNEES RETOURNEES ===============
			...
			@$r=array("ttarif"=>$ttarif,
				...
				"tot_sans_tva_fc"=>$this->arrondie($this->nombre_format_2($tot_sans_tva)*$taux[0]['Usd_fc']),
				
				"tva"=>$tva,
				"tva_fc"=>($this->nombre_format_2($tva)*$taux[0]['Usd_fc']),
	...
	function immatriculation($id)
	{
		$s="select * from rva_facturation2.immatriculation where Id_imm='$id'"; 
		$e=$this->cnx->query($s);
		$t=$e->fetch();
		$r=["detail"=>$t["Code_imm"]];
		return $r;
	}
	function mouvEscalesA($mouv)
	{
		$s="select * from rva_facturation2.escale where Sens='A' and Id_mouv='$mouv'";
		$e=$this->cnx->query($s);
		$t=$e->fetchAll();
		return $t;
	}
	function mouvEscalesD($mouv)
	{
		$s="select * from rva_facturation2.escale where Sens='D' and Id_mouv='$mouv'";
		$e=$this->cnx->query($s);
		$t=$e->fetchAll();
		return $t;
	}
	...
	//================== TABLEAU DE BORD =========
	function tableau_de_bord($dt,$dt2)
	{
		$rda_mht=0;
		$rda_tva=0;
		$rva_ttc=0;

		$s="select * from rva_facturation2.paiement_facture where Date_paie between '$dt' and '$dt2'";
		$e=$this->cnx->query($s);
		$t=$e->fetchAll();
		$n=count($t);
		if($n==0)
		...
		}else
		{
			...
			$s_rda_supp="select * from rva_facturation2.rda_suppl where Date_paie between '$dt' and '$dt2'";
		...
		//================= 	IDF ==============================
		$sidf="select * from rva_facturation2.idf_paiement where idf_paiement.Date_enr between '$dt' and '$dt2'";
		...
		//================= ACCES  =================================
		$sacces="select * from rva_facturation2.acces where Date_perc between '$dt' and '$dt2' and Type_acc!=1 and Type_acc!=6";
		...
		//================= PARKING  =================================
		$sparking="select * from rva_facturation2.acces where Date_perc between '$dt' and '$dt2' and Type_acc=1";
		...
		//================= HANDLING  =================================
		$shand="select * from rva_facturation2.acces where Date_perc between '$dt' and '$dt2' and Type_acc=6";
		...
		}else
		{
			foreach ($thand as $row) {
				$hand_mht+=$row["Mt_acc"];
				$hand_tva+=$this->tva($row["Mt_acc"]); 
				$hand_ttc+=$row["Mt_acc"]+$this->tva($row["Mt_acc"]);
			}
		}
