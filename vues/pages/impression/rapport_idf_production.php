<?php
	include("../../../manager/bd/cnx.php");
	include("../../../manager/parametre/parametre.php");
	class m extends main{}
	$m=new m();
	
	$dt1=$_REQUEST['dt1'];
	$dt2=$_REQUEST['dt2'];
	$client=$_REQUEST["client"];
	if($client=='T')
	{
		$s="select * from rva_facturation2.facture_imprime 
			where Date_impr between '$dt1' and '$dt2' and ltrim(Num_facture) Like '%RDS%'";
			$e=$m->cnx->query($s);
			$row=($e->fetchAll());
			$n=count($row);
			$r=array();
	}else
	{
		$s="select * from rva_facturation2.idf_paiement 
			where 
				Client='$client' 
			and 
				Date_idf between '$dt1' and '$dt2' 
			order by Id_paie desc";
			$e=$m->cnx->query($s);
			$row=($e->fetchAll());
			$n=count($row);
			$r=array();
	}	
	if($n==0)
	{		
		echo "<div align='center' class='bold center green'>Aucune donnée trouvée à cette periode</div>";
	}else
	{
		require("../../../plugins/fpdf/fpdf.php");
		require("../../../plugins/chiffresEnLettres.php");
		
	
		class PDF extends FPDF
		{
			function Footer()
			{
				
			}
		}
		$taille=array(250,300);
		$pdf = new PDF('P','mm','A4');
		$pdf->AliasNbPages();
		$pdf->AddPage();
		$pdf->SetLeftMargin(15);
		$pdf->SetTopMargin(25);
		$pdf->SetFont('Arial','B',10);
		$pdf->Image('../../../images/entete_pdf.png',86,15,60,20);
		$npage=$pdf->PageNo();
		
		//$pdf->Line(15, 260, 190, 260);
		//$pdf->Line(15, 70, 190, 70);
		
		$pdf->SetFont('Arial','',8);
		/*$pdf->Text(20, 270, utf8_decode("N° TVA : 664 / 2012"));
		$pdf->Text(20, 275, utf8_decode("N° IMPOT : A1201438C"));*/
		
		$pdf->Text(10, 20, utf8_decode("REPUBLIQUE DEMOCRATIQUE DU CONGO"));
		$pdf->Text(10, 23, utf8_decode("REGIE DES VOIES AERIENNES SA"));
		$pdf->Text(10, 26, utf8_decode("AEROPORT INTERNATIONAL DE LA LUANO"));
		$pdf->Text(10, 29, utf8_decode("N° TVA : A0700324 L"));
		$pdf->Text(10, 32, utf8_decode("----------------------------------------------------------------------"));
		
		$pdf->Ln(28);
		$pdf->SetFont('Arial','BU',10);
		$pdf->Cell(180,5,("RAPPORT DE PRODUCTION DES IDF "),'','','C');	
		$pdf->Ln();
		$pdf->SetFont('Arial','BU',9);
		$pdf->Cell(180,5,("DU ".$m->Datemysqltofr($dt1)." AU ".$m->Datemysqltofr($dt2)),'','','C');	
			$pdf->Ln(10);
			$pdf->SetFont('Arial','',8);
			$pdf->Cell(15,5,("DATE"),'LTRB','','C');
			$pdf->Cell(40,5,("EXPLOITANT"),'LTRB','','C');
			
			$pdf->Cell(20,5,("MHT USD"),'TRB','','C');
			$pdf->Cell(20,5,("TVA (USD)"),'TRB','','C');	
			
			$pdf->Cell(20,5,("TTC USD"),'TRB','','C');	
			$pdf->Cell(40,5,("FACTURE"),'TRB','','C');	
			$mttusd=0;
			$mttcdf=0;
			$usd=0;
			$cdf=0;
			$arrondiecdf_total=0;
			$arrondieusd_total=0;
			$arrondieusd=0;
			$arrondiecdf=0;
			$tvausd_total=0;
			$tvacdf_total=0;
			$ttccdf_total=0;
			$ttcusd_total=0;
			$tot_sans_tva=0;
			$tot_avec_tva=0;
			foreach($row as $t)
			{
				@$mouv=$m->mouv($t['Mouv']);
				@$tot_sans_tva+=$t["Montant"];
				@$tva=$t["Tva"];
				$tot_avec_tva+=$tva;
				$pdf->Ln();
				if(@$t["Monn"]=="USD")
				{
					$mhtusd=$t['Mt'];
					$mhtcdf=0;
					$tvacdf=0;
					
						$tvausd=0;
					$mttusd=ceil($mhtusd+$tvausd);
					$mttcdf=0;
					$arrondieusd=$mttusd-($mhtusd+$tvausd);
					$arrondiecdf=0;
				}else
				{
					@$mhtcdf=$t['Mt'];
					$mhtusd=0;
					$tvausd=0;
					
						$tvacdf=0;
					$mttcdf=($mhtcdf+$tvacdf);
					$mttusd=0;
					$arrondiecdf=0;
					$arrondieusd=0;
				}
				
				@$pdf->Cell(15,5,$m->Datemysqltofr($t['Date_impr']),'LTRB','','C');
				@$pdf->Cell(40,5,($mouv["nom_cli"]),'LTRB','','C');
				@$pdf->Cell(20,5,$t["Montant"],'TRB','','R');
				$pdf->Cell(20,5,$m->arrondie($tva),'TRB','','R');	
				@$pdf->Cell(20,5,($m->arrondie($t["Montant"]+$t["Tva"])),'TRB','','R');	
				@$pdf->Cell(40,5,$t["Num_facture"],'TRB','','C');	
				
				//=========== TOTAUX =============
				$usd=$usd+$mouv["tot_sans_tva"];
				$cdf=0;
				@$tvausd_total=$tvausd_total+$t["Tva"];
				$tvacdf_total=0;
				$arrondiecdf_total=0;
				$arrondieusd_total=$arrondieusd_total+$arrondieusd;
				$ttccdf_total=0;
				@$ttcusd_total=$ttcusd_total+($t["Montant"]+$t["Tva"]);
				
			}
				
				$pdf->Ln(10);
				$pdf->Cell(55,5,("TOTAL"),'LTRB','','C');
				
				$pdf->Cell(20,5,$m->arrondie($usd),'TRB','','R');
				$pdf->Cell(20,5,$m->arrondie($tvausd_total),'TRB','','R');	
				
				
				$pdf->Cell(20,5,$m->arrondie($ttcusd_total),'TRB','','R');		
		$pdf->Output();
	}
?>