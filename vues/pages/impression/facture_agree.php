<?php
	...
	/*$pdf->Text(20, 275, utf8_decode("N° IMPOT : A1201438C"));*/
			//======================== TRAITEMENT DONNEES====================		
				...
				$s="select * from rva_facturation2.mouvement2,rva_facturation2.client,rva_facturation2.immatriculation 
					where 
			...
			//==================== ENTETE ====================================
				...
				$pdf->Cell(180,7, utf8_decode("FACTURE N°RVA 111.7/".$_SESSION['num_fact_agr']." /RAI/".$m->mois_chiffre($dt2).".".$m->annee($dt2)),'','','C'); 
				$pdf->Ln();
				...
				$pdf->Ln();
				$pdf->Cell(180,4, utf8_decode("LE MONTANT DES REDEVANCES DETAILLEES COMME SUITE POUR LE MOIS DE : ".strtoupper($m->mois_long($dt2))." ".$m->annee($dt1)),'','','L'); $pdf->Ln();
				$pdf->Ln(5);
			//================================================================
				$pdf->Ln();
				...
				$pdf->Cell(60,5, $m->arrondie($r['tot']),'LBR','','R'); 
				$pdf->Ln();
				if(trim($client["code_nat"])=="Z")
				{
					$tva=$m->tva($r["tot"]);
				}else
				{
				...
				$pdf->Cell(60,5, $m->arrondie($tva),'LBR','','R'); 
				$pdf->Ln();
				$pdf->Cell(100,5, "TOTAL TOUTE TAXE COMPRISE",'LRB','','L'); 
				$pdf->Cell(60,5,$m->arrondie(ceil($r['tot']+$tva)),'LBR','','R'); 
				...
				$pdf->Ln();
				$v=$lettre->Conversion(ceil($r['tot']+$tva));
