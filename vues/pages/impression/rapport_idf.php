<?php
	...
	{
		$s="select * from rva_facturation2.idf_paiement where Client='$client' and Date_idf between '$dt1' and '$dt2' order by Id_paie desc";
			$e=$m->cnx->query($s);
			$row=($e->fetchAll());
			$n=count($row);
			$r=array();
	...
	{  
		...
		$pdf->Cell(180,5,("RAPPORT DE PAIEMENT DES IDF "),'','','C');	
		$pdf->Ln();
		...
		$pdf->Cell(180,5,("DU ".$m->Datemysqltofr($dt1)." AU ".$m->Datemysqltofr($dt2)),'','','C');	
			...
			$pdf->Cell(30,5,("EXPLOITANT"),'LTRB','','C');
			...
			foreach($row as $t)
			{
				...
				$client=$m->client($t['Client']);
				$pdf->Cell(15,5,$m->Datemysqltofr($t['Date_enr']),'LTRB','','C');
				$pdf->Cell(30,5,($client['nom_cl']),'LTRB','','C');
				...
				$pdf->Cell(20,5,$m->arrondie($mttusd),'TRB','','R');	
