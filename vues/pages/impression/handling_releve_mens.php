<?php
	...
	/*$pdf->Text(20, 275, utf8_decode("N° IMPOT : A1201438C"));*/
			//======================== TRAITEMENT DONNEES====================		
				$s="select * from handling_facturation where Handleur='$handleur' and Date_dep between '$du' and '$au'"; 
				$e=mysqli_query($bdd,$s); 
				$t=mysqli_fetch_array($e);	
				$handleur=handling_facture($t['Id_fact'],$bdd);
			//==================== ENTETE ====================================
				$pdf->AddPage();
				...
				$pdf->Cell(180,7, utf8_decode("FACTURE N°RVA 111.7/"."RDHM.FZQA/".$fact."/".mois_chiffre($du).".".annee($au)),'','','C'); 
				$pdf->Ln(7);
				...
				$pdf->Ln();$pdf->Ln();
				$pdf->MultiCell(210,4, utf8_decode("DOIT A LA REGIE DES VOIES AERIENNES POUR LA REDEVANCE D'ASSISTANCE AU SOL DES AERONEFS A L'AEROPORT INTERNATIONAL DE LUANO LE MONTANT CI-DESSOUS POUR LE MOIS DE : ".strtoupper(ucwords(mois_long($au)))),'','',''); 
				$pdf->Ln();
				$pdf->Ln();
			//================================================================
				$mt=0;
				do
				{
					$facture=handling_facture($t['Id_fact'],$bdd);
					$mt=$mt+$facture['mht'];
				...
				$pdf->Cell(60,8, arrondie($mt),'LRB','','R'); 
				$pdf->Ln();
				$pdf->Cell(100,8, "MONTANT TVA 16%",'LRB','','L'); 
				$pdf->Cell(60,8, arrondie($tva),'LRB','','R'); 
				$pdf->Ln();
				$pdf->Cell(100,8, "TOTAL MENSUEL",'LRB','','L'); 
				$pdf->Cell(60,8, arrondie(ceil($mttc)),'LRB','','R'); 
				$pdf->Ln();
...
//========================================== RELEVE ================================================================================
	...
	$pdf->Ln();
	$pdf->SetFont('Arial','',8);
	$pdf->Cell(270,7, utf8_decode("Période du : ".Datemysqltofr($du)." au ".Datemysqltofr($au)),'','','C'); 
	$pdf->Ln(7);
	$pdf->SetFont('Arial','B',9);
	$pdf->Ln(6);
	...
	$pdf->Cell(22,7, utf8_decode("MONT AA"),'TBR','','C');	
	...
	$pdf->Cell(35,7, utf8_decode("MONTANT TTC"),'TBR','','C');	
				$s2="select * from handling_facturation where Handleur='$p_handleur' and Date_dep between '$du' and '$au'"; 
	...
	do
	{
		$pdf->SetFont('Arial','',9);
		$pdf->Ln();
		$ligne=handling_facture($t2['Id_fact'],$bdd);
		$pdf->Cell(25,7, (($ligne['dt_dep'])),'LR','','L');	
		$pdf->Cell(35,7, ($ligne['heure_dep']),'R','','L');	
		$pdf->Cell(40,7, utf8_decode($ligne['client']),'R','','L');
		$pdf->Cell(30,7, utf8_decode($ligne['imm']),'R','','L');	
		$pdf->Cell(22,7, arrondie($ligne['aa_prix']),'R','','R');	
		$pdf->Cell(22,7, arrondie($ligne['ta_prix']),'R','','R');	
		$pdf->Cell(22,7, arrondie($ligne['fa_prix']),'R','','R');	
		$pdf->Cell(22,7, arrondie($ligne['mht']),'R','','R');	
		$pdf->Cell(20,7, arrondie($ligne['tva']),'R','','R');	
		$pdf->Cell(35,7, arrondie($ligne['mttc']),'R','','R');
	...
	$pdf->Cell(22,9, arrondie($t_aa),'TRBL','','R');	
	$pdf->Cell(22,9, arrondie($t_ta),'TRB','','R');	
	$pdf->Cell(22,9, arrondie($t_fa),'TRB','','R');	
	$pdf->Cell(22,9, arrondie($t_mht),'TRB','','R');	
	$pdf->Cell(20,9, arrondie($t_tva),'TRB','','R');	
	$pdf->Cell(35,9, arrondie($t_mttc),'TRB','','R');
