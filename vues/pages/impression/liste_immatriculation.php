<?php
	...
	$pdf->Text(20, 275, utf8_decode("N° IMPOT : A1201438C"));*/
			...
			$s="select * from rva_facturation2.immatriculation,rva_facturation2.client,rva_facturation2.type_avion 
				where 
					immatriculation.Code_pr=client.Id_cl and immatriculation.Type_av=type_avion.Id_typ 
				order by 
					client.Nom_cli";
			$e=$m->cnx->query($s);
			$t=($e->fetchAll());
			...
			foreach($t as $row)
			{
				$pdf->Cell(60,7, $row['Nom_cli'],'LRBT','','L');
				$pdf->Cell(25,7, $row['Code_imm'],'LRBT','','C');
				$pdf->Cell(25,7, $row['Libelle_typ'],'RBT','','C');
				$pdf->Cell(25,7, $row['Poids'],'LRBT','','C');
				$pdf->Cell(20,7, "",'RBT','','C');
				$pdf->Cell(15,7, "",'RBT','','C');
				$pdf->Cell(15,7, "",'RBT','','C');
				$pdf->Ln();
