...
{	
	...
	$url="facture_cash_re.php?num_mouv=".$num_mouv;
	echo ('<script type="application/javascript" language="javascript">'.
		'location.href="'.$url."'".
		'</script>');
	echo(utf8_decode("<center><div align='center' style='text-align:center; font-weight:bold; font-size:25px; background-color:red; width:65%; border-radius:20px; padding:20px; color:white'>D&eacutesol&eacute, cette facture est d&eacuteja imprim&eacutee</div></center>"));
	echo ("<div style='text-align:center; padding:20px 0 0 0'>Voulez-vous la reimprimer <a href='".$url."'>Oui</a></div>");
	
}else
{
	$m->journal_insert($id_us,'impression_facture',$num_mouv);
	$mouvement=$m->mouv($num_mouv);
	$s="select * from rva_facturation2.facture_imprime order by Id_fact desc"; 
	$e=$m->cnx->query($s); $t=$e->fetchAll(); $n=count($t);
		if($n==0){$num=1;}else
		{
			$num=$t[0]['N_ordre']+1;
		}
		$num_facture=$m->format_nbre($num)."/RDA.FZQA/".date('n.Y');
	//====================================== DONNEES DE LA TABLE ========================
	...
	$pdf->Text(10, 32, utf8_decode("----------------------------------------------------------------------"));
			...
			//=========================================INSERTION DU N° DE LA FACTURE
				...
				if($n==0)
				{
					/*$s="insert into rva_facturation2.facture_imprime 
					(
						Date_impr,Heure_impr,N_ordre,Mouv,Num_form,Montant	
					)
					values(
					'$dt','$heure','$num','$num_facture','','$_mouv','V','FACT')";*/
				}
			//=============================================================
	...
	//=================== 2e Tableau====================================
			...
			$pdf->Cell(70,7,$tot_sans_tva_fc.' FC','LTBR','','C');
	...
	//==================== INSERT DANS LA TABLE DES FACTURES IMPRIMEES==================
		...
		$s="insert into rva_facturation2.facture_imprime (Date_impr,Heure_impr,N_ordre,Mouv,Num_facture,Montant,Tva,Taux,Statut,Valide,Id_us) values ('$dt','$heure','$num','$num_mouv','$num_facture','$montant','$tva','$taux','R','O','$id_us')";
		$m->cnx->exec($s);
...
if($mouvement['tot_red_fret']>0)
{
	...
	$pdf->Text(10, 32, utf8_decode("----------------------------------------------------------------------"));
			...
			//=========================================INSERTION DU N° DE LA FACTURE
				...
				if($n==0){ $num=1; }else { $num=$t[0]['N_ordre']+1; }
				;
				$num_facture=$m->format_nbre($num)."/RDS.FZQA/".date('n.Y');				
			...
			foreach($row as $t)
			{
				...
				$pdf->Cell(9,6, "A",'L','','C');  
				$pdf->Cell(14,6, $m->Datemysqltofr($mouvement['ta'][0]['Date_mouv']),'L','','L');
				$pdf->Cell(11,6, $mouvement['ta'][0]['Heure_mouv'],'L','','R');
				$pdf->Cell(12,6, $t['Code_pt'],'L','','C');
				$pdf->Cell(12,6, $t['Ad'],'L','','R');
				$pdf->Cell(12,6, $t['Ch'],'L','','R');
				$pdf->Cell(12,6, $t['Inf'],'L','','R');
			...
			foreach($row as $t)
			{
				...
				$pdf->Cell(9,6, "D",'L','','C');  
				$pdf->Cell(14,6, $m->Datemysqltofr($mouvement['td'][0]['Date_mouv']),'L','','L');
				$pdf->Cell(11,6, $mouvement['td'][0]['Heure_mouv'],'L','','R');
				$pdf->Cell(12,6, $t['Code_pt'],'L','','C');
				$pdf->Cell(12,6, $t['Ad'],'L','','R');
				$pdf->Cell(12,6, $t['Ch'],'L','','R');
				$pdf->Cell(12,6, $t['Inf'],'L','','R');
	...
	//=================== 2e Tableau====================================
			...
			$fret_d=$mouvement['red_fret_d'];
			$fret_total=$idf_fret_arr+$idf_fret_dep;
			$fret_total_fc=$fret_total*$mouvement['taux'][0]['Usd_fc'];
			$idf=(ceil($fret_total));
			$idf_fc=$idf*$mouvement['taux'][0]['Usd_fc'];
			//******************************************************
	...
	//============== insertion dans sql=========================
	$s="insert into rva_facturation2.facture_imprime (Date_impr,Heure_impr,N_ordre,Mouv,Num_facture,Montant,Tva,Taux,Statut,Valide,Id_us) values('$dt','$heure','$num','$num_mouv','$num_facture','$fret_total',0,0,'I','O','$id_us')";
	//$s="insert into rva_facturation2.facture_imprime (Date_impr,Heure_impr,N_ordre,Mouv,Num_facture,Montant,Tva,Taux,Statut,Valide,Id_us) values('$dt','$heure','$num','$num_mouv','$num_facture',0,0,0,'I','O','$id_us')";
