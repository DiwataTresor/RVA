<?php
	...
	$pdf->Text(20, 30, utf8_decode("----------------------------------------------------------------------"));
			...
			$pdf->Ln(20);
			$pdf->Cell(270,6, utf8_decode("RELEVE DES FACTURES NON PAYEES"),'','','C'); 
			$pdf->Ln();
			...
			$pdf->Ln(); $pdf->Ln();
	//================================= FACTURE==================
			$pdf->Cell(18,5, utf8_decode("DATE MOUV"),'TRL','','C'); 
			$pdf->Cell(14,5, utf8_decode("HEURE"),'TRL','','C');
			$pdf->Cell(14,5, utf8_decode("PROV"),'TRL','','C');
			$pdf->Cell(14,5, utf8_decode("DEST"),'TRL','','C');
			$pdf->Cell(40,5, utf8_decode("NBRE PAX"),'TRL','','C');
			$pdf->Cell(14,5, utf8_decode("FRET"),'TRL','','C');
			$pdf->Cell(110,5, utf8_decode("REDEVANCES"),'TRL','','C');
			
			$pdf->Cell(60,5, utf8_decode("TOTAL RED"),'TRL','','C');
			$pdf->Ln();
			$pdf->Cell(18,5, utf8_decode(""),'RBL','','C'); 
			$pdf->Cell(14,5, utf8_decode(""),'RBL','','C');
			$pdf->Cell(14,5, utf8_decode(""),'RBL','','C');
			$pdf->Cell(14,5, utf8_decode(""),'RBL','','C');
			$pdf->Cell(10,5, utf8_decode("AD"),'RBL','','C');
			$pdf->Cell(10,5, utf8_decode("CH"),'RBL','','C');
			$pdf->Cell(10,5, utf8_decode("INF"),'RBL','','C');
			$pdf->Cell(10,5, utf8_decode("PEC"),'RBL','','C');
			$pdf->Cell(14,5, utf8_decode("KG"),'RBL','','C');
			...
			$pdf->Cell(10,5, utf8_decode("SUR"),'RBL','','C');
			...
			$pdf->Cell(20,5, utf8_decode("MHT"),'RBL','','C');
			$pdf->Cell(20,5, utf8_decode("TVA"),'RBL','','C');
			$pdf->Cell(20,5, utf8_decode("MTTC"),'RBL','','C');
	//================================ SQL==================================
			/*$s="select * from 
	...
	//=============================== CONTENU DU TABLEAU=================================
			...
			if($n!==0)
			{
				...
				foreach($t as $row)
				{
					...
					//================ TABLE ESCALEQ==================	
						...
						$sd="select *, format(Date_mouv,'%d/%m/%Y') as dt_mouv 
							from 
								...
								rva_facturation2.pt_emplacement 
							where 
								mouvement2.Pt=pt_emplacement.Id_pt and Sens='D' 
							and 
								Num_mouv=$num_mouv";
						$ed=$m->cnx->query($sd);
						$td=($ed->fetchAll());
					...
					$pdf->Cell(144,8, utf8_decode("POIDS MAX. AU DECOLLAGE : ".$_m['ta'][0]['Poids']),'TBR','','L'); 
					$pdf->SetFont('Arial','',7);
					...
					foreach($row2 as $t2)
					{
						/************* check dans la table de paiement **********************/
							...
							$echeck=$m->cnx->query($check); $tcheck=$echeck->fetchAll(); $ncheck=count($tcheck);
							if($ncheck!==0){
							...
							}else
							{
								...
								/*=====================================*/
									...
									$pdf->Cell(18,5, utf8_decode($m->Datemysqltofr($mouvement2['ta'][0]['Date_mouv'])),'RL','','C'); 
									$pdf->Cell(14,5, utf8_decode($m->Heureformat($mouvement2['ta'][0]['Heure_mouv'])),'RL','','C');
									$pdf->Cell(14,5, utf8_decode($mouvement2['ville_arr']),'RL','','C');
									$pdf->Cell(14,5, utf8_decode("FZQA"),'RL','','C');
									$pdf->Cell(10,5, utf8_decode($a_somme_ad),'RL','','R');
									$pdf->Cell(10,5, utf8_decode($a_somme_ch),'RL','','R');
									$pdf->Cell(10,5, utf8_decode($a_somme_inf),'RL','','R');
									$pdf->Cell(10,5, utf8_decode($a_somme_pec),'RL','','R');
									$pdf->Cell(14,5, utf8_decode($a_somme_loc),'RL','','R');
									$pdf->Cell(10,5, utf8_decode($m->arrondie($mouvement2['red_route_a'])),'RL','','R');
									$pdf->Cell(10,5, utf8_decode($m->arrondie($mouvement2['red_att'])),'RL','','R');
									$pdf->Cell(10,5, utf8_decode($m->arrondie($mouvement2['red_bal_a'])),'RL','','R');
									$pdf->Cell(10,5, utf8_decode($m->arrondie($mouvement2['red_fret_a'])),'RL','','R');
									$pdf->Cell(10,5, utf8_decode($m->arrondie($mouvement2['red_pass_a'])),'RL','','R');
									$pdf->Cell(10,5, utf8_decode($m->arrondie($mouvement2['red_stat'])),'RL','','R');
									$pdf->Cell(10,5, "",'RL','','R');
									$pdf->Cell(10,5, utf8_decode(""),'RL','','R');
									$pdf->Cell(10,5, "",'RL','','R');
									$pdf->Cell(10,5, "",'RL','','R');
									$pdf->Cell(10,5, "",'RL','','R');
									
									$mht=$mouvement2['red_route_a']+$mouvement2['tot_red_att']+$mouvement2['red_bal_a']+$mouvement2['red_fret_a']+$mouvement2['red_stat'];
									$tva=$m->tva($mht);
									$total_ligne_a=$mht+$tva;

												
									$pdf->Cell(20,5, utf8_decode($m->arrondie($mht)),'RL','','R');
									$pdf->Cell(20,5, utf8_decode(""),'RL','','R');
									$pdf->Cell(20,5, utf8_decode(""),'RL','','R');
								...
								$pdf->Ln();	
									...
									@$pdf->Cell(14,5, utf8_decode($m->Heureformat($mouvement2['td'][0]['Heure_mouv'])),'RL','','C');
									$pdf->Cell(14,5, utf8_decode("FZQA"),'RL','','C');
									$pdf->Cell(14,5, utf8_decode($mouvement2['ville_dep']),'RL','','C');
									$pdf->Cell(10,5, utf8_decode($d_somme_ad),'RL','','R');
									$pdf->Cell(10,5, utf8_decode($d_somme_ch),'RL','','R');
									$pdf->Cell(10,5, utf8_decode($d_somme_inf),'RL','','R');
									$pdf->Cell(10,5, utf8_decode($d_somme_pec),'RL','','R');
									$pdf->Cell(14,5, utf8_decode($d_somme_loc),'RL','','R');
									$pdf->Cell(10,5, utf8_decode($m->arrondie($mouvement2['red_route_d'])),'RL','','R');
									$pdf->Cell(10,5, "",'RL','','R');
									$pdf->Cell(10,5, utf8_decode($m->arrondie($mouvement2['red_bal_d'])),'RL','','R');
									$pdf->Cell(10,5, utf8_decode($m->arrondie($mouvement2['red_fret_d'])),'RL','','R');
									$pdf->Cell(10,5, utf8_decode($m->arrondie($mouvement2['red_pass_d'])),'RL','','R');
									$pdf->Cell(10,5, "",'RL','','R');
									$pdf->Cell(10,5, utf8_decode($m->arrondie($mouvement2['red_surete'])),'RL','','R');
									$pdf->Cell(10,5, utf8_decode($m->arrondie($mouvement2['red_securite'])),'RL','','R');
									$pdf->Cell(10,5, utf8_decode($m->arrondie($mouvement2['red_formu'])),'RL','','R');
									$pdf->Cell(10,5, utf8_decode($m->arrondie($mouvement2['red_compt'])),'RL','','R');
									$pdf->Cell(10,5, utf8_decode($m->arrondie($mouvement2['red_assantinc'])),'RL','','R');
								...
								$tot_mht=$tot_mht+($mht+$mht2);
								$tot_tva=$tot_tva+$tva2;
								
								$total_ligne_d=ceil($mht+$mht2+$tva2);
								...
								/*================ SOUS TOTAL DEPART=============================*/
										...
										$tot_mttc_ligne=ceil($tot_mht_ligne+$tot_tva_ligne);
										//$tot_mttc_ligne=52;
										$general_ligne=$general_ligne;
								/*========================================================*/
					...
					$pdf->Ln();
					$pdf->SetFont('Arial','',6);
					$pdf->Cell(60,5, "SOUS TOTAL",'LTRB','','R'); 
					$pdf->Cell(10,5, $ad_ligne,'LTRB','','R'); 
					$pdf->Cell(10,5, $ch_ligne,'LTRB','','R'); 
					$pdf->Cell(10,5, $inf_ligne,'LTRB','','R'); 
					$pdf->Cell(10,5, $pec_ligne,'LTRB','','R'); 
					$pdf->Cell(14,5, $m->arrondie($fret_kg_ligne),'LTRB','','R'); 
					$pdf->Cell(10,5, $m->arrondie($route_ligne),'RLT','','R');
					$pdf->Cell(10,5, $m->arrondie($atterrissage_ligne),'RLT','','R');
					$pdf->Cell(10,5, $m->arrondie($bal_ligne),'RLT','','R');
					$pdf->Cell(10,5, $m->arrondie($fret_ligne),'RLRT','','R');
					$pdf->Cell(10,5, $m->arrondie($pax_ligne),'RLT','','R');
					$pdf->Cell(10,5, $m->arrondie($stat_ligne),'RLT','','R');
					$pdf->Cell(10,5, $m->arrondie($surete_ligne),'RLT','','R');
					$pdf->Cell(10,5, $m->arrondie($securite_ligne),'RLT','','R');
					$pdf->Cell(10,5, $m->arrondie($formu_ligne),'RLT','','R');
					$pdf->Cell(10,5, $m->arrondie($compt_ligne),'RLT','','R');
					$pdf->Cell(10,5, $m->arrondie($ass_ligne),'RLT','','R');
					$pdf->Cell(20,5, utf8_decode($m->arrondie($tot_mht_ligne)),'RLT','','R');
					$pdf->Cell(20,5, utf8_decode($m->arrondie($tot_tva_ligne)),'RLT','','R');
					$pdf->Cell(20,5, utf8_decode($m->arrondie($tot_mttc_ligne)),'RLT','','R');
				...
				$pdf->SetFont('Arial','',6);
				$pdf->Cell(18,6, "",'','','C');
				$pdf->Cell(42,6, "TOT.RES INTER. US $",'TRLB','','L');
				$pdf->Cell(10,6, $m->arrondie($ad),'TRLB','','R');
				$pdf->Cell(10,6, $m->arrondie($ch),'TRLB','','R');
				$pdf->Cell(10,6, $m->arrondie($inf),'TRLB','','R');
				$pdf->Cell(10,6, $m->arrondie($pec),'TRLB','','R');
				$pdf->Cell(14,6, $m->arrondie($fret),'TRLB','','R');
				$pdf->Cell(10,6, $m->arrondie($route),'TRLB','','R');
				$pdf->Cell(10,6, $m->arrondie($atterrissage),'TRLB','','R');
				$pdf->Cell(10,6, $m->arrondie($bal),'TRLB','','R');
				$pdf->Cell(10,6, $m->arrondie($fret),'TRLB','','R');
				$pdf->Cell(10,6, $m->arrondie($pax),'TRLB','','R');
				$pdf->Cell(10,6, $m->arrondie($stat),'TRLB','','R');
				$pdf->Cell(10,6, $m->arrondie($surete),'TRLB','','R');
				$pdf->Cell(10,6, $m->arrondie($securite),'TRLB','','R');
				$pdf->Cell(10,6, $m->arrondie($formu),'TRLB','','R');
				$pdf->Cell(10,6, $m->arrondie($compt),'TRLB','','R');
				$pdf->Cell(10,6, $m->arrondie($ass),'TRLB','','R');
				$pdf->SetFont('Arial','',7);
				$pdf->Cell(20,6, $m->arrondie(ceil($tot_mht)),'TRLB','','R');
				$pdf->Cell(20,6, $m->arrondie(ceil($tot_tva)),'TRLB','','R');
				$pdf->Cell(20,6, $m->arrondie(ceil($tot_mttc)),'TRLB','','R');
