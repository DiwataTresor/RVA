<?php
	...
	$handling_facture=handling_facture($id_fact,$bdd);
	@session_start();
	...
	$pdf->Text(10, 32, utf8_decode("----------------------------------------------------------------------"));
			//=========================== CREATION N° FACTURE===============================
				$s="select * from handling_num_fact where Id_fact='$id_fact'";
			...
			$pdf->Cell(45,6, $handling_facture['heure_dep'],'LBR','','C');
	...
	//=================== 2e Tableau====================================
			...
			if($handling_facture["aa"]=="O")
			{
				$pdf->Ln();	
				$pdf->SetFont('Arial','',9);	
				$pdf->Cell(40,6, "",'','','C');  		
				$pdf->Cell(61,6, "ASSISTANCE ENGINS,CARGO,AUTO ASSISTANCE",'L','','C');  
				$pdf->Cell(45,6, arrondie($handling_facture['aa_prix']),'LR','','R');		
			}
			...
			if($handling_facture["ta"]=="O")
			{
				...
				$pdf->Cell(45,6, arrondie($handling_facture['ta_prix']),'LR','','R');		
			}
			if($handling_facture["fa"]=="O")
			{
				...
				$pdf->Cell(45,6, arrondie($handling_facture['fa_prix']),'LR','','R');		
			}
			...
			$pdf->Cell(40,7,arrondie($handling_facture['mht']),'RTB','','R');	
			
			$pdf->Ln();
			$pdf->Ln();
			$pdf->Cell(40,6, "",'','','C');  
			$pdf->Cell(40,7,"TVA : ",'LTB','','C');	
			$pdf->Cell(66,7,arrondie($handling_facture['tva']),'RTB','','R');	
			...
			$pdf->Cell(40,7,arrondie($handling_facture['mttc']),'RTB','','R');	
	...
	//************************Fin Bon de sortie
			...
			$pdf->Text(70, 260,"  ".arrondie($handling_facture['mttc']));
