<?php
	...
	$m=new m();
	$s="select TOP 100 journal.Date_jrn,journal.Heure_jrn, journal.Id_us
		from 
			rva_facturation2.journal,rva_facturation2.utilisateur 
		where 
			journal.Id_us=utilisateur.Id_us and Type_op='mouvement' 
		order by 
			Date_jrn desc";
	$e=$m->cnx->query($s);
							...
							echo ("<table class='w3-table w3-table-all'>
									...
									foreach($row as $t)
									{
										...
										echo ("<tr>
											...
											<td>".$t['Heure_jrn']."</td>
											<td>".$mouv['ta'][0]['Nom_cli']." (".$mouv['ta'][0]['Code_imm']." )"."</td>
